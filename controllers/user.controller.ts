...
export const verifyForgotPassword = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (existUser) {
            ...
            await User.update({ password: hashedPassword }, { where: { id: existUser.id } });
...
export const updatePassword = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (userJson) {
            ...
            user = await User.findOne({ where: { id: userData.id }, attributes: ["id", "password"] });
        } else {
            user = await User.findOne({ where: { id: userId }, attributes: ["id", "password"] });
        }
